...
- ⚡ **Asynchronous Execution:** Launches processes on background threads to keep the macOS user interface butter-smooth.
- 🪵 **Real-Time Logs:** Captures stdout and stderr streams in a retro, copyable terminal log window inside the UI.
- 🤖 **AI-Powered Diagnostics:** Streams logs to a Cloudflare Worker backend to analyze crashes, return confidence levels, and suggest exact fixes.
- 🧩 **Runtime Versatility:** Supports both local bundled Wine runtimes and system-level applications like *Wine Devel.app*.
